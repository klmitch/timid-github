...
Execute the timid-github test suite.
...
if [ ${venv} = yes ]; then
    ...
    echo "Installing timid-github setup in the virtual environment"
    python ${dir}/setup.py install
    if [ $? -ne 0 ]; then
        echo "Failed to install timid-github setup in virtual environment" >&2
        exit 1
    fi
...
if [ ${pep8} != only ]; then
	...
	options="${options} --cover-package=timid_github"
    ...
    if [ $? -ne 0 ]; then
	echo "Tests on timid-github failed" >&2
...
if [ ${pep8} != no ]; then
    ...
    echo
    run_pep8 ${dir}/timid_github.py ${dir}/test_timid_github.py
    if [ $? -ne 0 ]; then
